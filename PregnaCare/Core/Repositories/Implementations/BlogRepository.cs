...
namespace PregnaCare.Core.Repositories.Implementations
{
    public class BlogRepository : GenericRepository<Blog, Guid>, IBlogRepository
    {
        ...
        public async Task<IEnumerable<BlogDTO>> GetAllActiveBlogAdminAsync(string type)
        {
        ...
        {
            var blogList = await _context.Blogs
                .Where(b => b.IsDeleted == false && b.IsVisible == true && b.Type.ToLower() == type.ToLower() && b.Status == StatusEnum.Approved.ToString())
                .Include(x => x.User)
                .OrderByDescending(b => b.UpdatedAt)
                .ThenByDescending(b => b.CreatedAt)
                .Select(blog => new BlogDTO
